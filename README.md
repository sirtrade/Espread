...
незаметно вплетает их в следующие статьи, тренировки и чат-викторины
бота по расписанию интервальных повторений (SRS). Слово поднимается по
лестнице интервалов `1 → 3 → 7 → 14 → 30 → 60 → 120` дней; ошибка (слово
снова помечено или неверный ответ) мягко откатывает его на пару ступеней,
а слово, пережившее всю лестницу до верхней ступени, автоматически
становится `learned`.
...
- **Банк слов**: экран «Tu banco de palabras» показывает слова с переводами и
  контекстом по статусам (в прогрессе / в очереди / выучено / отброшено);
  вкладка «в очереди» — принятые слова, запаркованные до освобождения слота в
  активном пуле (`active_pool_limit`, промоушен FIFO). Статус можно менять
  вручную через `PATCH /api/bank/:id`.

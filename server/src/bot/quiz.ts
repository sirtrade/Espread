...
export function registerQuizHandlers(bot: Bot): void {
  bot.callbackQuery(/^pq:(\d+):(\d+):(\d+)$/, async (ctx) => {
    ...
    const answerLine = item
      ? `${item.term}${item.translation ? ` — ${item.translation}` : ""}`
      : "";
